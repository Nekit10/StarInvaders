...
public class ArmourScreen implements Screen {
    ...
    public ArmourScreen(StarInvaders game, MainMenuScreen menu) throws IOException {
        this.menu = menu;
        ...
        label = prop.getProperty("armour.label");
        menuLables = prop.getProperty("armour.elements").split(";");

        prop.load(new FileInputStream("properties/armour.properties"));
    ...
    public void render(float delta) {
        ...
        int npos = (int) game.settingsGameData.get("armour", -5);
        ...
        if ((Gdx.input.isKeyPressed(Input.Keys.ENTER) || Gdx.input.isKeyPressed(Input.Keys.SPACE)) && TimeUtils.nanoTime() - login > 500000000) {
            game.settingsGameData.set("armour", (pos + 1));
            game.setScreen(menu);
        }
    }
