...
public class AudioVolumeScreen implements Screen {
    ...
    public void render(float delta) {
        ...
        try {
            for (int i = 0; i < menuLables.length; i++) {
                if (Float.parseFloat(menuLables[i]) == (float) game.settingsGame.get("volume", 0f))
                    npos = i;
            }
        } catch (ClassCastException e) {
            for (int i = 0; i < menuLables.length; i++) {
                if (Float.parseFloat(menuLables[i]) == ((Integer) game.settingsGame.get("volume", 0f)).floatValue())
                    npos = i;
            }
        }
        ...
        if ((Gdx.input.isKeyPressed(Input.Keys.ENTER) || Gdx.input.isKeyPressed(Input.Keys.SPACE)) && TimeUtils.nanoTime() - login > 500000000) {
            game.settingsGame.set("volume", Float.parseFloat(menuLables[pos]));
            game.setScreen(menu);
        }
    }
