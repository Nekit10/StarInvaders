...
public class SModeScreen implements Screen {
    ...
    public SModeScreen(StarInvaders game, SettingsRenderScreen menu) throws IOException {
        this.menu = menu;

        game.log.Log("Initializing Smode screen", LogSystem.INFO);
        ...
        label = prop.getProperty("settings.smode.label");
        menuLables = prop.getProperty("settings.smode.elements").split(";");

        prop.load(new FileInputStream("properties/settings/smode.properties"));
    ...
    public void render(float delta) {
        ...
        int npos = (boolean) game.settingsGame.get("smode", false) ? 0 : 1;
        ...
        if ((Gdx.input.isKeyPressed(Input.Keys.ENTER) || Gdx.input.isKeyPressed(Input.Keys.SPACE)) && TimeUtils.nanoTime() - login > 500000000) {
            game.settingsGame.set("smode", pos == 0);
            try {
