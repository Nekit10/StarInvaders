...
public class BrightnessScreen implements Screen {
    ...
    public BrightnessScreen(StarInvaders game, SettingsVideoScreen menu) throws IOException {
        this.menu = menu;

        game.log.Log("Initializing brightness screen", LogSystem.INFO);
        ...
        label = prop.getProperty("settings.brightness.label");
        menuLables = prop.getProperty("settings.brightness.elements").split(";");

        prop.load(new FileInputStream("properties/settings/brightness.properties"));
    ...
    public void render(float delta) {
        ...
        try {
            for (int i = 0; i < menuLables.length; i++) {
                if (Float.parseFloat(menuLables[i]) == (float) game.settingsGame.get("brightness", 0f))
                    npos = i;
            }
        } catch (ClassCastException e) {
            for (int i = 0; i < menuLables.length; i++) {
                if (Float.parseFloat(menuLables[i]) == ((Integer) game.settingsGame.get("brightness", 0f)).floatValue())
                    npos = i;
            }
        }
        ...
        if ((Gdx.input.isKeyPressed(Input.Keys.ENTER) || Gdx.input.isKeyPressed(Input.Keys.SPACE)) && TimeUtils.nanoTime() - login > 500000000) {
            game.settingsGame.set("brightness", Float.parseFloat(menuLables[pos]));
            game.setScreen(menu);
        }
    }
