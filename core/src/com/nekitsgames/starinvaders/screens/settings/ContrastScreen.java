...
public class ContrastScreen implements Screen {
    ...
    public ContrastScreen(StarInvaders game, SettingsVideoScreen menu) throws IOException {
        this.menu = menu;

        game.log.Log("Initializing contrast screen", LogSystem.INFO);
        ...
        label = prop.getProperty("settings.contrast.label");
        menuLables = prop.getProperty("settings.contrast.elements").split(";");

        prop.load(new FileInputStream("properties/settings/contrast.properties"));
    ...
    public void render(float delta) {
        ...
        try {
            for (int i = 0; i < menuLables.length; i++) {
                if (Double.parseDouble(menuLables[i]) == (double) game.settingsGame.get("contrast", 0.0))
                    npos = i;
            }
        } catch (ClassCastException e) {
            for (int i = 0; i < menuLables.length; i++) {
                if (Double.parseDouble(menuLables[i]) == ((Integer) game.settingsGame.get("contrast", 0.0)).doubleValue())
                    npos = i;
            }
        }
