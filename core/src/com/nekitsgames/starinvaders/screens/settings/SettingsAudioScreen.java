...
public class SettingsAudioScreen implements Screen {
    ...
    public SettingsAudioScreen(StarInvaders game, SettingsScreen menu) throws IOException {
        this.menu = menu;
        ...
        label = prop.getProperty("settings.group.audio.label");
        menuLables = prop.getProperty("settings.group.audio.elements").split(";");
    ...
    public void render(float delta) {
        ...
        if ((Gdx.input.isKeyPressed(Input.Keys.ENTER) || Gdx.input.isKeyPressed(Input.Keys.SPACE)) && TimeUtils.nanoTime() - login > 500000000)
            switch (pos) {
                case 0:
                    try {
                        game.setScreen(new AudioVolumeScreen(game, this));
                    } catch (IOException e) {
                        e.printStackTrace();
